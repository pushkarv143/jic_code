...
mysql -u root -p school_management_system  < 23_credential_reset_permission.sql
```
...
| `23_credential_reset_permission.sql` | Adds `STUDENT_CREDENTIALS_RESET`, granted to **SUPER_ADMIN alone**, behind the Resend Credentials button on the student form. A permission rather than `hasRole('SUPER_ADMIN')` for the same reason as `PRIVILEGE_MANAGE`: every other authority here became configurable, and an endpoint pinned to a role name is the one a school cannot delegate without a code change — the behaviour today is identical, the difference shows up the day they want their office manager to do it. Deliberately separate from `STUDENT_UPDATE`: a clerk correcting a spelling should not also be able to lock a student out and mail their password. The action regenerates the temporary password, **keeps the username** (it is the student's identity, a lost password says nothing about it, and regenerating would collide and come back as `pushkarv1` — a different person to read), re-arms the forced first-login reset, revokes every live session, and sends the credentials by email and SMS. Re-runnable. |
